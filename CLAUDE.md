...
- Pending context cells now have a source audit at `data/cities/hyderabad/pending-context-sources.json`: 75 context cells audited, 47 matched to Telangana TGRAC `HMDA ORR GHMC Village Boundary`, and 28 still require non-HMDA/village boundary sourcing before promotion.
- TGRAC-matched pending cells also have reproducible local boundary evidence at `data/cities/hyderabad/tgrac-pending-village-boundaries.geojson`: 45 unique official village polygons cover the 47 matched pending rows because some pending cells share the same official FID. These are source evidence only, not PlotDNA scored market polygons.
- Hyderabad pending hover/search cards must show their own detail state for every visible pending polygon: official village/mandal/district plus revenue/division/admin/DMV/FID when available, or `needs non-HMDA boundary source` when TGRAC does not cover that exact pending cell.
