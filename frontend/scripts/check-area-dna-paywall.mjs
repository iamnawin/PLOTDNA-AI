...
assert(areaDetail.includes('Preview the DNA, then download the lifetime PDF.'), 'export panel must explain the direct PDF test path')
...
assert(areaDetail.includes('onUnlock={() => downloadInstantPdf('), 'timed lock must download the Rs 99 PDF directly while payment is disabled for testing')
