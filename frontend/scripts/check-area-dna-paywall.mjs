...
assert(areaDetail.includes('Preview the DNA, then unlock lifetime app access and PDF.'), 'export panel must explain timed preview access')
assert(areaDetail.includes('Preview time complete. Unlock lifetime access and PDF.'), 'export panel must collapse into lock-state copy after preview')
...
assert((timedGate.match(/Rs 99/g) ?? []).length === 1, 'timed lock must show exactly one Rs 99 price card')
