...
assert(!rainbowButton.includes('cta-reflection-sheen'), 'rainbow CTA must not keep the older white reflection sheen layer')
assert(!rainbowButton.includes('via-white/24'), 'rainbow CTA must not include the previous gradient reflection sweep')
...
assert(areaDetail.includes("openEmailGateForPdf('area_dna_export_cta')"), 'unverified export CTA must open the access flow before direct PDF testing')
assert(areaDetail.includes("downloadInstantPdf('area_dna_verified_download')"), 'verified users must download PDF only from an explicit button')
...
assert(exportPanelMatch[0].includes('if (accessUnlocked)'), 'export panel must replace pricing copy after access is active')
assert(!areaDetail.includes("downloadInstantPdf('paid_access_claimed')"), 'claiming paid access must not auto-download the PDF')
...
assert(areaDetail.includes('const isTimedPreviewLocked ='), 'area detail must centralize the timed preview lock state')
assert(areaDetail.includes('{!isTimedPreviewLocked && ('), 'top export card must hide while the timed lock overlay is active')
...
assert(areaDetail.includes('Shortlist answer') && areaDetail.includes('Map proof') && areaDetail.includes('What to verify'), 'timed lock must preview included report features without adding another price card')
...
const buyerChecklist = areaDetail.indexOf('BUYER_DUE_DILIGENCE_CHECKLIST.map')
const exportPanel = areaDetail.indexOf('<ReportExportPanel')
const accessGate = areaDetail.indexOf('<TimedDnaAccessGate')
const buyerJourney = areaDetail.indexOf('<BuyerJourneyScreens')
...
assert(buyerJourney > -1 && buyerJourney < verdict, 'new buyer journey must appear before the deep report')
assert(verdict < exportPanel && exportPanel < accessGate, 'export CTA should appear after the AI verdict and before the timed preview gate')
assert(accessGate < buyerChecklist, 'timed gate should protect deep sections while hiding the separate top export offer after timeout')
