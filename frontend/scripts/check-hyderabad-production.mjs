...
const pendingPromotionReportPath = path.join(repoRoot, 'data', 'cities', 'hyderabad', 'pending-promotion-report.json')
...
const pendingInfrastructureSignals = fs.existsSync(pendingInfrastructureSignalsPath)
  ...
  : null
const pendingPromotionReport = fs.existsSync(pendingPromotionReportPath)
  ? JSON.parse(fs.readFileSync(pendingPromotionReportPath, 'utf8').replace(/^\uFEFF/, ''))
  : null
...
assert(pendingSignalInventory.summary?.signalDeckReadyCount === pendingSignalInventory.areaInventories.filter(inventory => inventory.signalDeckReady).length, 'pending signal inventory ready summary must match ready rows')
...
assert(pendingScoringReadiness.summary?.promotionReadyCount === pendingScoringReadiness.areaAudits.filter(audit => audit.promotionReady).length, 'pending scoring-readiness promotion summary must match ready rows')
assert(pendingPromotionReport?.schemaVersion === 1, 'Hyderabad pending promotion report must exist')
assert(pendingPromotionReport.areaPromotionRows?.length === contextFeatures.length, `pending promotion report must cover every context cell: expected ${contextFeatures.length}, found ${pendingPromotionReport?.areaPromotionRows?.length ?? 0}`)
assert(pendingPromotionReport.summary?.promotionReadyCount === pendingScoringReadiness.summary?.promotionReadyCount, 'pending promotion report must match scoring-readiness promotion count')
assert(pendingPromotionReport.promotionReadyRows?.length === pendingPromotionReport.summary?.promotionReadyCount, 'promotion-ready report rows must match summary count')
const promotionRowsMissingEvidence = pendingPromotionReport.areaPromotionRows.filter(row => requiredEvidence.some(key => !row.verifiedEvidence?.includes(key)) && row.promotionReady)
assert(promotionRowsMissingEvidence.length === 0, `pending promotion report marked rows ready without full evidence: ${promotionRowsMissingEvidence.map(row => row.slug).join(', ')}`)
assert(pendingPromotionReport.summary?.verifiedEvidenceCounts?.price_band === verifiedPriceSignals.length, 'pending promotion report must preserve verified price signal count')
assert(pendingPromotionReport.summary?.verifiedEvidenceCounts?.infrastructure === verifiedInfrastructureSignals.length, 'pending promotion report must preserve verified infrastructure signal count')
...
assert(mapView.includes('estimatedScore'), 'estimated scored cells must be explicitly flagged for subdued map styling')
assert(mapView.includes("['==', ['get', 'estimatedScore'], 1], 0.12"), 'estimated scored cells must stay below primary scored fill strength')
assert(mapView.includes("['==', ['get', 'broadGenerated'], 1], 0.13"), 'large generated scored cells must stay below primary scored fill strength')
assert(mapView.includes("['==', ['get', 'estimatedScore'], 1], 0.46"), 'estimated scored cell borders must be visibly lower-confidence')
assert(mapView.includes("['==', ['get', 'broadGenerated'], 1], 0.58"), 'large generated scored cell borders must be readable without dominating')
assert(mapView.includes('Limited source depth'), 'estimated scored cells must explain their lower-confidence score on hover')
