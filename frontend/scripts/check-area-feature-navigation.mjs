...
assert(areaDetail.includes('useScroll({ container: featureTabsRef })'), 'feature navigator must drive progress from the tab scroller position')
assert(areaDetail.includes('useTransform(scrollXProgress, [0, 1], [\'0%\', \'100%\'])'), 'feature navigator must map scroll progress into indicator width')
