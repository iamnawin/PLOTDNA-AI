...
assert(areaDetail.includes('const { scrollYProgress } = useScroll()'), 'feature navigator must drive progress from page scroll position')
assert(areaDetail.includes('useTransform(scrollYProgress, [0, 1], [\'14%\', \'100%\'])'), 'feature navigator must map page scroll progress into a visible indicator width')
