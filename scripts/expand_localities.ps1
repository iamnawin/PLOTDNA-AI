...
# Expand Hyderabad localities from 52 to 144
# Adds 92 new localities across 5 zones

$base = "C:\Users\Naveen\OneDrive\Desktop\PlotDNA"

# --- 1. locality data: slug, display name, center [lat,lng], dLat, dLng ---
$newLocalities = @(
  # CENTRAL (18)
...
function Rnd([double]$v) { [math]::Round($v, 3) }

function Get-Poly($lat, $lng, $dLat, $dLng) {
  $p = @(
    "[$(Rnd($lat+$dLat)), $(Rnd($lng))]",
    "[$(Rnd($lat+$dLat*0.7)), $(Rnd($lng+$dLng*0.7))]",
    "[$(Rnd($lat)), $(Rnd($lng+$dLng))]",
    "[$(Rnd($lat-$dLat*0.7)), $(Rnd($lng+$dLng*0.7))]",
    "[$(Rnd($lat-$dLat)), $(Rnd($lng))]",
    "[$(Rnd($lat-$dLat*0.7)), $(Rnd($lng-$dLng*0.7))]",
    "[$(Rnd($lat)), $(Rnd($lng-$dLng))]",
...
# Build locality JSON entries
$locEntries = $newLocalities | ForEach-Object {
  $poly = Get-Poly $_.lat $_.lng $_.dLat $_.dLng
  "  {`n    `"slug`": `"$($_.slug)`",`n    `"name`": `"$($_.name)`",`n    `"center`": [$($_.lat), $($_.lng)],`n    `"polygon`": $poly`n  }"
}
...
# Remove last char which should be ]
if ($trimmed[-1] -eq ']') { $trimmed = $trimmed.Substring(0, $trimmed.Length - 1).TrimEnd() }
$newContent = $trimmed + "," + "`n" + ($locEntries -join ",`n") + "`n]"
Set-Content $locFile $newContent -Encoding UTF8 -NoNewline
Write-Host "localities.json updated: $($newLocalities.Count) new entries"

# --- Update clusters.json ---
$clustersFile = "$base\data\cities\hyderabad\clusters.json"
$clusters = Get-Content $clustersFile -Raw | ConvertFrom-Json

foreach ($cluster in $clusters) {
  $zone = $cluster.zone.ToLower()
  $newSlugs = ($newLocalities | Where-Object { $_.zone -eq $zone } | ForEach-Object { $_.slug })
  $existing = [System.Collections.Generic.List[string]]($cluster.localitySlugs)
  foreach ($s in $newSlugs) { $existing.Add($s) }
  $cluster.localitySlugs = $existing.ToArray()
}

$clusters | ConvertTo-Json -Depth 10 | Set-Content $clustersFile -Encoding UTF8
...
Write-Host "`nAll data files updated. Now run build to verify."
